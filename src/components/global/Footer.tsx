...
function Footer(props: SettingsPayload) {
      ...
      <div className="container">
        ...
        <div className="mt-12 flex flex-col items-start justify-between space-y-6 md:grid md:grid-cols-2 md:gap-x-12 md:space-y-0 lg:grid-cols-3 lg:space-y-0">
          ...
          <div className="col-span-1 flex flex-col space-y-2 md:order-1">
            <h3 className="text-lg font-semibold text-rope mb-4">About Us</h3>
            {props.footerText && (
              <div className="footer-cutomportabletext max-w-sm">
                <CustomPortableText value={props.footerText} />
              </div>
            )}
            {props.socialLinks && (
          ...
          <div className="col-span-1 flex flex-col space-y-2 md:order-3">
            <h3 className="text-lg font-semibold text-rope mb-4">
              Quick Links
            </h3>
            {props.footerMenuItems && (
              <div className="flex flex-col space-y-2">
                {props.footerMenuItems.map((link) => {
                  return (
                    <div key={link._key}>
                      {link.reference ? (
                        <Link
          ...
          {/* Contact */}
          <div className="cols-span-1 flex flex-col space-y-2 md:order-2">
            <h3 className="text-lg font-semibold text-rope mb-4">Contact</h3>
            {props.postalAddress?.address && (
              <div itemScope itemType="https://schema.org/Organization">
                ...
                >
                  ...
                  {props.postalAddress.suburb && (
                    <span itemProp="addressLocality">
                      {props.postalAddress.suburb}
                      {props.postalAddress.state && ', '}
                    </span>
                  )}
                  {props.postalAddress.state && (
                    <span className="uppercase" itemProp="addressRegion">
                      {props.postalAddress.state}
                      {props.postalAddress.postcode && ' '}
                    </span>
                  )}
                  {props.postalAddress.postcode && (
            ...
            {props.geolocation?.lat && (
              ...
              >
                <meta
                  itemProp="latitude"
                  content={props.geolocation?.lat.toString()}
                />
                <meta
                  itemProp="longitude"
                  content={props.geolocation?.lng.toString()}
                />
                <a
                  href={`https://www.google.com/maps/search/?api=1&query=${props.geolocation?.lat},${props.geolocation?.lng}`}
                  target="_blank"
                  rel="noopener noreferrer"
        ...
        <div className="flex items-center justify-center text-center py-4">
          <p className="mt-4 text-sm">
            Copyright © {new Date().getFullYear()} {props.siteTitle} . All
            Rights Reserved.
          </p>
        </div>
      </div>
